...
def moabc(actGraph, candidates, SQ, MCN, constraints):
    ...
    def getNeighbor(cp):
        ...
        neighbor.G.nodes[act]["service"] = sample([neighbor for neighbor in L if neighbor != s], 1)[0]
        return {"cp": neighbor, "fitness": fit(neighbor), "functions": functions(neighbor), "limit": 0,
                "probability": 0}
    ...
    for itera in range(MCN):
        ...
        t = 0
        while t < SN:
            if solutionsList[s]["probability"] <= uniform(min([solutionsList[x]["probability"] for x in range(SN)]),
                                                          max([solutionsList[x]["probability"] for x in range(SN)])):
                t += 1
                cp = solutionsList[s]
                while 1:
                    new = getNeighbor(cp["cp"])
            ...
            if solutionsList[pick]["limit"] >= SQ:
                while 1:
                    ...
                    if verifyConstraints(cp.cpQos(), constraints):
                        solutionsList[pick] = {"cp": cp, "fitness": fit(cp), "functions": functions(cp), "limit": 0,
                                               "probability": 0}
                        break

...
def test(num_act, num_candidates, constraints, weightList):
    ...
    for i in range(10):
        ...
        _ , fit = hybrid.ABCgenetic(actGraph, candidates, workers=50, onlookers=50, scouts=100, SQ=5, MCN=100, SN=100, minQos=minQos, maxQos=maxQos, constraints=constraints, weightList=weightList)
    ...
    with open('dataset.csv', mode='a') as file:
        ...
        file_writer.writerow([str(num_act), str(num_candidates), '10','50', '50', '100', '5', '100', '100', str(y), str(opt), str(y / opt) ,z])

...
def test(num_act, num_candidates, constraints, weightList):
    ...
    for i in range(10):
        ...
        _ , fit = hybrid.ABCgenetic(actGraph, candidates,SQ=50, MCN=300, SN=200, minQos=minQos, maxQos=maxQos, constraints=constraints, weightList=weightList)
    ...
    with open('dataset.csv', mode='a') as file:
        ...
        file_writer.writerow([str(num_act), str(num_candidates), '10','50', '300', '200', str(y), str(opt), str(y / opt) ,z])

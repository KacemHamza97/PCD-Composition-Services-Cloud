...
def test(num_act, num_candidates, constraints, weightList):
    ...
    for i in range(10):
        ...
        _ , fit = hybrid.ABCgenetic(actGraph, candidates, workers=200, onlookers=200, scouts=100, SQ=10, MCN=100, SN=400, minQos=minQos, maxQos=maxQos, constraints=constraints, weightList=weightList)
    ...
    with open('dataset.csv', mode='a') as file:
        ...
        file_writer.writerow([str(num_act), str(num_candidates), '10','200', '200', '100', '10', '100', '400', str(y), str(opt), str(y / opt) ,z])

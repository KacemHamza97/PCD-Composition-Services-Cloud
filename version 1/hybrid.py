...
def ABCgenetic(actGraph, candidates, WBee , OBee , SBee , SQ, MCN, SN, p, minQos, maxQos, weightList):
    # initializing
    solutions = list()
    fitnessList = list()
    ...
    for i in range(SN):
        sol = cloud.randomCompositionPlan(actGraph, candidates)
        fit = f(sol, minQos, maxQos, weightList)
        ...
        for bee_num in range(SBee):
            ...
            if limit[i] == SQ:  # scouts bee condition verified
                ...
                if itera >= CP:
                    ...
                    # Scouting
                    w = cloud.randomCompositionPlan(actGraph, candidates)
